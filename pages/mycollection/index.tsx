            ...
            <section className="sliderView m-5 m-md-5 ">
                ...
                <div className="ocassion-slider mt-4 mt-lg-5 ml-4">
                    <Slider {...ocassionSliderSetting} className="">
                        {typeof window !== undefined  && myCollectionMenu.map((item: any, index: number) => {
                            return (<>
                                <div className="collection position-relative "> <a href="/shop">
                                    <div className="imgsec m-5 m-lg-5">
                                        <img className="w-110" src={item.collection_image} alt="" />
                                    </div>
                                    <div className="text-center mt-4">
                                        <h3 className="fs-20  font-sb text-color-2 mb-0">{item.collection_name}</h3>
                                        <p className="fs-12 font-r text-color-1">{item.created_data}</p>
                                    </div> </a>
                                </div></>)
                        })}
                    </Slider>
