...
const CapturePayment = ({ data }: any) => {
    ...
    const [paymentStatus, setPaymentStatus] = useState<string>(data?.paymentStatus)
                ...
                .then((response: any) => {
                    if (response?.data) {
                        ...
                        OrderTakingAppService.getInstance("")
                            .updateOrder(requestDATA, response?.data?.data?.merchantPaymentRefId)
                            .then((response: any) => {
        ...
        <>
            {
                paymentStatus == "AUTH_APPROVED" && (
                    <button className="btn btn-xs" onClick={() => handleCapturePayment()}>Capture Payment</button>
                )
            }
            {
